...
const NUTRITION =
  typeof module !== "undefined" && module.exports ? require("./nutrition-data.js") : NUTRITION_DATA;
...
function sameDimensionRatio(from, to) {
  ...
  for (const measures of [MASS_IN_GRAMS, VOLUME_IN_TSP]) {
    if (Object.hasOwn(measures, from) && Object.hasOwn(measures, to))
      return measures[from] / measures[to];
  }
...
function ingredientGrams(ingredient) {
  ...
  if (ingredient.preparation && ingredient.preparation !== reference.preparation) {
    throw new Error(
      `Preparation mismatch for ${ingredient.name}: expected ${reference.preparation}`,
    );
  }
  const unit = nutritionUnit(ingredient.unit);
  if (Object.hasOwn(MASS_IN_GRAMS, unit)) return ingredient.amount * MASS_IN_GRAMS[unit];
  if (Object.hasOwn(reference.gramsPerUnit, unit))
    return ingredient.amount * reference.gramsPerUnit[unit];
  if (Object.hasOwn(VOLUME_IN_TSP, unit)) {
    const basis = Object.keys(reference.gramsPerUnit).find((key) =>
      Object.hasOwn(VOLUME_IN_TSP, key),
    );
    if (basis)
      return ingredient.amount * sameDimensionRatio(unit, basis) * reference.gramsPerUnit[basis];
  }
  throw new Error(
    `Unsupported unit "${ingredient.unit}" for ${ingredient.name}. Use grams or a documented portion.`,
  );
}
...
function expandIngredient(ingredient) {
  ...
  return blend.components.map(([name, grams]) => ({
    ...
    amount: grams * ratio,
    unit: "g",
    preparation: NUTRITION[name].preparation,
...
function validateRecipeIngredients(recipes) {
  ...
  Object.values(recipes)
    ...
    .forEach((recipe) => {
      ...
      if (
        ...
        invalidFields.add(recipe.name || recipe.id || "(unnamed)");
      if (
        !recipe.macros ||
        Object.values(recipe.macros).some((value) => !Number.isFinite(value) || value < 0) ||
        recipe.macros.protein > 200 ||
        recipe.macros.calories > 2500 ||
        recipe.macros.carbs > 400 ||
        recipe.macros.fat > 200
      )
        invalidNutrition.add(recipe.name || recipe.id);
  ...
  return {
    ...
    invalidNutrition: [...invalidNutrition],
  };
}
...
function buildRecipeLibrary() {
  const breakfast = [
    recipe(
      ...
      [
        "Weigh the yogurt and dry oats and powder as listed, then stir together.",
        "Top with blueberries and peanut butter.",
        "Chill overnight or portion immediately.",
      ],
      1,
    ),
    recipe(
      ...
      [
        "Warm the ready-to-eat turkey and wilt the raw spinach in a nonstick skillet with a splash of water.",
        "Add the raw eggs and egg whites, scramble until set, then stir in the listed cheese.",
        "Cool before packing.",
      ],
      2,
    ),
    recipe(
      ...
      [
        ...
        "Pack avocado separately and add after reheating.",
      ],
      2,
    ),
    recipe(
      ...
      [
        "Weigh potatoes raw, combine the listed masala spice components, and roast on parchment at 425 F until tender.",
        "Sear paneer in a nonstick skillet and wilt spinach with a splash of water; no unlisted oil is included.",
        "Top with a cooked egg before portioning.",
      ],
      3,
    ),
    recipe(
      ...
      [
        item("cottage cheese", 1, "cup"),
        item("whole grain bread", 2, "slices"),
        item("blueberries", 0.75, "cup"),
        item("peanut butter", 1, "tbsp"),
      ],
      [
        "Toast the bread until crisp.",
        "Spread cottage cheese and the measured peanut butter across the toast.",
        "Pack blueberries separately and add immediately before serving.",
      ],
      2,
    ),
    recipe(
      ...
      "american",
      "chicken",
      "Chicken Sweet Potato Breakfast Hash",
      [
        item("chicken breast", 4.5, "oz"),
        item("sweet potato", 1, "count"),
        item("bell peppers", 1, "cup"),
        item("spinach", 1, "cup"),
        item("paprika", 1, "tsp"),
      ],
      [
        "Cook chicken without extra oil to 165 F, rest it, and weigh the cooked portion.",
        "Dice the raw sweet potato and pepper; roast on parchment at 425 F until tender.",
        "Wilt spinach with water, season with paprika, combine, and cool in shallow containers within 2 hours.",
      ],
      2,
    ),
    recipe(
      ...
      "mexican",
      "beef",
      "Beef Black Bean Breakfast Bowl",
      [
        item("lean ground beef", 4, "oz"),
        item("black beans", 0.65, "cup"),
        item("whole eggs", 1, "count"),
        item("salsa", 0.25, "cup"),
        item("spinach", 1, "cup"),
      ],
      [
        "Cook ground beef to 160 F and the egg until set, using a food thermometer for the beef.",
        "Warm cooked drained beans and wilt spinach with a splash of water.",
        "Portion with salsa on the side and refrigerate or freeze within 2 hours.",
      ],
      3,
    ),
    recipe(
      ...
      "mediterranean",
      "turkey",
      "Turkey Hummus Breakfast Pita",
      [
        item("turkey slices", 4, "oz"),
        item("pita", 1, "count"),
        item("hummus", 3, "tbsp"),
        item("cucumber", 1, "cup"),
        item("spinach", 1, "cup"),
      ...
      [
        "Warm the ready-to-eat turkey and pita separately.",
        "Wash and dry cucumber and spinach, then keep them cold.",
        "Pack hummus and vegetables separately; assemble after reheating the turkey and pita.",
      ],
      2,
    ),
    recipe(
      ...
      "asian",
      "fish",
      "Salmon Edamame Breakfast Rice",
      [
        item("salmon fillet", 4.5, "oz"),
        item("brown rice", 0.75, "cup"),
        item("edamame", 0.5, "cup"),
        item("spinach", 1.5, "cups"),
        item("soy sauce", 1, "tbsp"),
      ],
      [
        "Cook salmon to 145 F and weigh the cooked edible portion.",
        "Cook brown rice in water and measure it cooked; warm edamame and wilt spinach.",
        "Add measured soy sauce, portion into shallow containers, and refrigerate or freeze within 2 hours.",
      ],
      4,
    ),
    recipe(
      ...
      "asian",
      "vegetarian",
      "Savory Tofu Quinoa Breakfast Bowl",
      [
        item("extra firm tofu", 7, "oz"),
        item("quinoa", 0.7, "cup"),
        item("spinach", 1.5, "cups"),
        item("snap peas", 1, "cup"),
        item("ginger", 1, "tbsp"),
      ],
      [
        "Drain and weigh tofu, then brown it in a nonstick pan with a splash of water.",
        "Cook quinoa in water and measure it cooked; steam snap peas and wilt spinach.",
        "Stir in measured ginger, portion into shallow containers, and chill within 2 hours.",
      ],
      2,
    ),
    recipe(
      ...
      "mediterranean",
      "vegetarian",
      "Chickpea Avocado Breakfast Pita",
      [
        item("chickpeas", 0.85, "cup"),
        item("pita", 1, "count"),
        item("avocado", 0.5, "count"),
        item("cucumber", 1, "cup"),
        item("lemon", 0.5, "count"),
      ],
      [
        "Warm cooked drained chickpeas and the pita separately.",
        "Dice cucumber and combine it with the measured fresh lemon juice.",
        "Keep avocado and cucumber cold and assemble the pita immediately before eating.",
      ],
      1,
    ),
    recipe(
      ...
      "italian",
      "vegetarian",
      "Egg White Marinara Breakfast Toast",
      [
        item("egg whites", 1, "cup"),
        item("whole grain bread", 2, "slices"),
        item("marinara sauce", 0.35, "cup"),
        item("spinach", 1.5, "cups"),
        item("mozzarella cheese", 0.2, "cup"),
      ],
      [
        "Cook egg whites until fully set and wilt spinach with a splash of water.",
        "Toast bread and warm the measured marinara.",
        "Top with egg whites, spinach, and mozzarella; cool leftovers in shallow containers within 2 hours.",
      ],
      2,
    ),
    recipe(
      ...
      "indian",
      "vegetarian",
      "Masala Lentil Potato Breakfast Bowl",
      [
        item("lentils", 0.85, "cup"),
        item("baby potatoes", 5, "oz"),
        item("spinach", 2, "cups"),
        item("plain Greek yogurt", 120, "g"),
        item("masala spices", 1, "tsp"),
      ],
      [
        "Cook lentils in water, drain, and measure them cooked.",
        "Weigh potatoes raw and roast on parchment at 425 F; wilt spinach with water and the measured spice blend.",
        "Combine the hot ingredients and pack yogurt separately to add after reheating.",
      ],
      1,
    ),
    recipe(
      ...
      "mexican",
      "turkey",
      "Turkey Salsa Quinoa Breakfast Bowl",
      [
        item("lean ground turkey", 4.5, "oz"),
        item("quinoa", 0.7, "cup"),
        item("corn", 0.6, "cup"),
        item("salsa", 0.3, "cup"),
        item("avocado", 0.25, "count"),
      ...
      [
        "Cook ground turkey to 165 F, measured with a food thermometer, then weigh the cooked portion.",
        "Cook quinoa in water and measure it cooked; warm and drain the corn.",
        "Portion with salsa, keep avocado separate, and chill in shallow containers within 2 hours.",
      ],
      2,
    ),
  ];
...
function carbStep(name) {
  if (name.includes("pasta"))
    return `Weigh ${name} dry, then boil in water until al dente and drain. The listed grams are dry pasta, not cooked weight.`;
  if (name.includes("potato"))
    return `Weigh ${name} raw, cut evenly, and roast at 425 F on parchment until tender; no extra oil is included.`;
  if (name.includes("naan") || name.includes("pita"))
    return `Weigh ${name} as sold, warm briefly, then pack separately.`;
  if (name.includes("cauliflower rice"))
    return `Weigh raw cauliflower as listed, rice it, and steam or cook in a nonstick pan with a splash of water.`;
  if (name.includes("beans") || name.includes("chickpeas") || name.includes("lentils"))
    return `Cook ${name} in water until tender, drain, and measure the listed cooked amount. Cooked amounts are not dry amounts.`;
...
function produceStep(name) {
  if (INGREDIENT_BLENDS[name])
    return `Combine the listed ingredients marked "for ${name}". Keep slaw raw; cook fajita or stir-fry vegetables in a nonstick pan with water, without unlisted oil.`;
  if (name.includes("romaine") || name.includes("spinach"))
    return `Measure ${name} raw, wash and dry it, and keep it cold or wilt it with a little water before serving.`;
  if (["corn", "peas", "edamame"].includes(name))
    return `Cook ${name}, drain, then measure the listed cooked quantity before portioning.`;
...
function proteinStep(type) {
  ...
  if (type === "beef")
    return "Cook ground beef to 160 F; cook steak to 145 F and rest it for 3 minutes. Use a food thermometer, add no extra oil, then weigh the listed cooked quantity.";
  if (type === "turkey")
    return "Cook 93% lean ground turkey without extra oil to 165 F, measured with a food thermometer, then weigh the listed cooked crumbles.";
  if (type === "fish")
    return "Cook salmon to 145 F, measured with a food thermometer, and weigh the cooked edible portion. For canned tuna, drain and weigh the solids; the reference can is 165 g drained.";
...
function sauceStep(name) {
  ...
  if (name.includes("seasoning") || name === "ginger" || name === "garlic")
    return `Stir the listed ${name} into the pan with a splash of water, then toss with the cooked ingredients.`;
  if (name === "lemon")
    return "Use the listed weight of fresh lemon juice after reheating; lemon quantities refer to juice, not whole-fruit weight.";
  if (name.includes("hummus")) return `Pack the listed ${name} separately and add after reheating.`;
  if (name.includes("cheese"))
    return `Add the listed ${name} to the hot meal or keep it separate until serving.`;
  if (name === "olive oil")
    return "Drizzle the listed olive oil over the assembled meal; this measured oil is included in the nutrition totals.";
...
function proteinItem(type, label, ounces) {
  ...
  const map = {
    ...
    fish: [
      item(
        dinner ? "salmon fillet" : "canned tuna",
        dinner ? rounded : 1.7,
        dinner ? "oz" : "cans",
      ),
    ],
...
function recipe(label, cuisine, proteinType, name, ingredients, steps, cost = 2) {
  const normalized = {
    id: `${label}-${cuisine}-${proteinType}-${name}`
      .toLowerCase()
      .replace(/[^a-z0-9]+/g, "-")
      .replace(/^-|-$/g, ""),
    label,
  ...
  normalized.allergens = [
    ...new Set(
      normalized.ingredients.flatMap((ingredient) =>
        (INGREDIENT_TAGS[ingredient.name] || []).filter((tag) =>
          ["dairy", "eggs", "fish", "legumes"].includes(tag),
        ),
      ),
    ),
  ].sort();
  ...
  normalized.searchText =
    `${name} ${ingredients.map((ingredient) => ingredient.name).join(" ")}`.toLowerCase();
  return normalized;
}
...
function macrosForIngredient(ingredient) {
  ...
  return Object.fromEntries(
    Object.entries(entry.macros).map(([key, value]) => [key, value * factor]),
  );
}

function supplementalPowderIngredient(proteinGrams) {
  if (!Number.isFinite(proteinGrams) || proteinGrams < 0)
    throw new Error("Invalid supplemental protein amount");
  const ref = NUTRITION["whey protein powder"];
  return {
    name: "whey protein powder",
    amount: (proteinGrams / ref.macros.protein) * ref.reference.amount,
    unit: "g",
...
if (recipeDataIssues.missingMacros.length) {
  console.error(
    `PrepFit: missing nutrition references for: ${recipeDataIssues.missingMacros.join(", ")}. Macro totals for affected recipes will be wrong.`,
  );
}
if (recipeDataIssues.missingCategory.length) {
  console.error(
    `PrepFit: missing CATEGORY_BY_INGREDIENT entries for: ${recipeDataIssues.missingCategory.join(", ")}. These ingredients will be grouped under "Other" in the grocery list.`,
  );
}
