...
test("mobile layout exposes the plan shortcut without horizontal overflow", async ({ browser }) => {
  ...
  const firstCard = page.locator("#meal-plan .meal-card").first();
  await expect(firstCard.getByRole("button", { name: /^Increase/ })).toBeVisible();
  const quickSwap = firstCard.getByRole("button", { name: /^Swap/ });
