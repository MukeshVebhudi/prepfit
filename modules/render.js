...
export function createRenderer({
  ...
  const budgetLabel = (value) =>
    ({ standard: "Standard", budget: "Budget", "high-protein": "High protein" })[value] ||
    "Standard";
  ...
  function renderSummary(settings, plan, averages, warning) {
    ...
    const status = outside.length
      ? {
          kind: outside.some((result) => result.kind === "under") ? "under" : "over",
          label: `${outside.length} ${plural("target", outside.length)} outside range`,
        }
      : { kind: "near", label: "All targets near" };
    dom.summaryTitle.textContent = `${settings.days}-day ${settings.mealMode} plan`;
    const activeMealCount = plan.days.reduce(
      (count, day) => count + day.meals.filter((meal) => !meal.removed).length * settings.people,
      0,
    );
    dom.summaryText.textContent =
      warning ||
      `${status.label}. ${settings.people} ${plural("person", settings.people)} with ${activeMealCount} planned meals.`;
    ...
    const stats = [
      ...
      [
        ...
        settings.powderProtein
          ? `${settings.supplementMode === "custom" ? settings.supplementLabel : "Whey protein powder"}: ${settings.powderProtein}g protein/day`
          : "Disabled",
      ],
      ...
      [
        "Target status",
        outside.length ? `${outside.length} outside tolerance` : "Within tolerance",
      ],
    ];
    dom.summaryStats.innerHTML = stats
      .map(
        ([label, value]) =>
  ...
  function renderPrepSchedule(plan, settings) {
    ...
    const meals = plan.days
      ...
      .filter((meal) => !meal.removed && !seen.has(meal.name) && seen.add(meal.name));
    const ingredients = new Set(meals.flatMap((meal) => meal.ingredients.map((item) => item.name)));
    const hasSauces = [...ingredients].some((name) =>
      /sauce|salsa|hummus|tzatziki|oil|yogurt/i.test(name),
    );
    const hasGrains = [...ingredients].some((name) => categoryByIngredient[name] === "Grains");
    const hasProduce = [...ingredients].some((name) =>
      ["Produce", "Frozen"].includes(categoryByIngredient[name]),
    );
    const proteinNames = [...ingredients].filter(
      (name) => categoryByIngredient[name] === "Protein",
    );
    const blocks = [
      {
        title: "Cook proteins",
  ...
  function renderFavorites() {
    ...
    dom.favoritesList.innerHTML = names.length
      ? `<ul>${names.map((name) => `<li>${escapeHtml(name)}</li>`).join("")}</ul>`
      : `<p class="empty-state">Saved meals will appear here and get priority in future plans.</p>`;
  }
