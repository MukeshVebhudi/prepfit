...
export function createPlanner({
  ...
  function recipeEligible(recipe, type, settings) {
    const cuisineMatch =
      settings.cuisine === "random" || recipe.cuisine === settings.cuisine || type === "Breakfast";
    const proteinMatch =
      settings.meat === "vegetarian"
        ? recipe.proteinType === "vegetarian" &&
          !recipe.ingredients.some((ingredient) =>
            (ingredientTags[ingredient.name] || []).some((tag) => tag === "meat" || tag === "fish"),
          )
        : type === "Breakfast" || recipe.proteinType === settings.meat;
    const avoids = settings.excluded.some((term) =>
      recipe.ingredients.some(
        (ingredient) =>
          ingredient.name.toLowerCase().includes(term) ||
          (ingredient.componentOf || "").includes(term) ||
      ...
      .sort((a, b) => {
        const score = (recipe) => {
          ...
          if (settings.calorieGoal)
            value += Math.abs(recipe.macros.calories - settings.calorieGoal / 3) * 0.12;
          if (settings.carbGoal)
            value += Math.abs(recipe.macros.carbs - settings.carbGoal / 3) * 0.2;
  ...
  function selectDayMeals(settings, usedNames, seed = 0) {
    ...
    let best = null;
    candidates[0].forEach((breakfast) =>
      candidates[1].forEach((lunch) =>
        candidates[2].forEach((dinner) => {
          ...
          const preference = rawMeals.reduce(
            (sum, meal, index) =>
              sum + recipeScore(meal, mealTypes[index], settings, usedNames, seed, index),
            0,
          );
  ...
  function buildPlan(settings, options = {}) {
    ...
    const supplementTerms =
      settings.supplementMode === "custom"
        ? `${settings.supplementLabel || ""},${settings.supplementAllergens || ""}`.toLowerCase()
        : "whey protein powder,dairy";
    if (
      settings.powderProtein &&
      settings.excluded.some((term) => supplementTerms.includes(term))
    ) {
      const label =
        settings.supplementMode === "custom" ? "configured supplement" : "supplemental whey";
      return {
        days,
        missingTypes: [],
        conflict: `The ${label} conflicts with your avoided ingredients or allergen tags. Set supplemental protein to zero or change the supplement details; restrictions have not been relaxed.`,
      };
    }
  ...
  function formatTargetDelta(result) {
    if (result.kind === "near")
      return `${Math.abs(Math.round(result.delta))}${result.unit} from target`;
  ...
  function buildWarning(plan, settings, averages = averageMacros(plan.days)) {
    ...
    if (settings.powderProtein > settings.dailyTarget)
      warnings.push(
        "The supplement alone exceeds the protein target; food portions stay at the minimum.",
      );
    if (outside.length) {
      const details = outside
        .map(
          (result) =>
            `${nutritionTargetLabel(result.nutrient).toLowerCase()} is ${formatTargetDelta(result)}`,
        )
        .join(", ");
      ...
      const reason =
        atMaximum && directions.has("under")
      ...
      warnings.push(
        `Closest plan within ${limits.min}×–${limits.max}× portion limits: ${details}.${reason}`,
      );
    }
